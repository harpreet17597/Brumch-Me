    ...
    <script>
        ...
        "use strict";
            ...
            $(document).ready(function ($){
                ...
                $(document).on('click','#form_submit',function (e){
                    ...
                    $.ajax({
                        ...
                        error:function(data){
                            ...
                            el.text('{{__('Login')}}');
                            $('div.alert.alert-danger').delay(1500).slideUp('slow',function(){$('div.alert.alert-danger').remove()});
                        },
                        success:function (data){
                            ...
                            if (data.status == 'ok'){
                                ...
                                $('div.alert.alert-'+data.type).delay(1500).slideUp('slow',function(){$('div.alert.alert-'+data.type).remove()});
                                location.reload();
                            }else{
                                erContainer.html('<div class="alert alert-'+data.type+'">'+data.msg+'</div>');
                                $('div.alert.alert-'+data.type).delay(1500).slideUp('slow',function(){$('div.alert.alert-'+data.type).remove()});
                ...
                $('.login-form-body input').on('keyup',function() {
                    if($('div.alert').length) {
                        $('div.alert').delay(1500).slideUp('slow',function(){$('div.alert').remove()});
                    }
                });
